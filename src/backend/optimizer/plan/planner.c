...
/*Hook variable for deal with plan node */
int anno_table_oid;
int data_anno_oid;
int summary_result_oid;
/*hook for debuging the removing plan node information */
bool debug_check_modify_plan_node=true;
bool debug_node=true;
...
static int check_init_plans(List * init_plans,int lcteid, int rcteid);
static void check_modify_plan_node(Plan * plantree, List * top_init_plans);

/*
 * for supporting plan node modify
 */
static void double_check(List * targetlist,char * targetone, char * targettwo, int * oid);
static void find_anno_table(List * targetlist);
static void remove_targetlist(List * targetlist);
static void top_remove_targetlist(List * targetlist); // server for top remove
static void basic_remove(Plan * plantree, bool * found);
static void debug_target_list(List * targetlist);
static void debug_plan_node(Node * node);
static int found_cte_scan(Node * plannode);
static void travel_plan_node(Node * plannode,List * top_init_plans);
static void travel_plan_node_tag(Node * plannode,int level);
/**
 * for support strategy 3
 */
static void top_change(Plan * plantree); // change the top node
static void sp_check_modify_remove_plan_node(Plan * plantree);
static void top_find_DActe(Plan * plantree,int * daid);
static void find_DACTE(List * top_init_plans, int * daid);
static void find_tempDACTE(List * top_init_plans,int * temp_daid);
//static void travel_sub_plans(List * subplans);
static Plan * return_DA_plan(List * subplans,int daid);
//static void top_travel_plan_node(Plan * plannode,List * top_init_plans);
static void changeDA(Plan * daplan);
...
/*
 * check and modify plan node entry
 */
void
debug_plan_node(Node * node){
	Node * nodetag=(Node *) nodeTag(node);
	if(node==NULL)
	{
		elog(INFO,"%s","NULL");
	}

	elog_node_display(INFO,"nodetag",nodetag,true);
}

int
check_init_plans(List * init_plans,int lcteid, int rcteid){
    int flag;
    ListCell * lf;
    flag=0;
    if(lcteid==0 && rcteid==0){
        return flag;/*not need change ptype!*/
    }
    if(init_plans==(List *)NULL){
    	return flag; // can't find init_plans.
    }

    foreach(lf,init_plans){
        Node * ipl=(Node *) lfirst(lf);
        if(IsA(ipl,SubPlan)){
            SubPlan * isub=(SubPlan *)ipl;
            if(find(isub->plan_name,"_anno")!=-1){
            	// found the end with __anno , we only allow the final one into this set!
                if(isub->plan_id==lcteid){
                    flag=1; /*left*/
                    break;
                }
                if(isub->plan_id==rcteid){
                    flag=2;
                    break;
                }
...
void
check_modify_plan_node(Plan * plantree, List * top_init_plans)
{

	int lcteid,rcteid;
	int flag;
    Node * inner_plan=(Node *)plantree->lefttree;
    Node * outer_plan=(Node *)plantree->righttree;

    lcteid=0;
    rcteid=0;

    if(!inner_plan|| !outer_plan) {return;}
    if(top_init_plans==(List *)NULL) {return;} //skip

    debug_plan_node(inner_plan);
    debug_plan_node(outer_plan);

    if(IsA(inner_plan,CteScan)){
            CteScan * inner_cteScan=(CteScan*)inner_plan;
            lcteid=(int)inner_cteScan->ctePlanId;
    }


    if(IsA(outer_plan,CteScan))
    {
            CteScan * outer_cteScan=(CteScan*)outer_plan;
            lcteid=outer_cteScan->ctePlanId;
    }




    flag=check_init_plans(top_init_plans,lcteid,rcteid);

    plantree->ptype=flag;
    if(debug_check_modify_plan_node){
    	if(flag!=0){
    		elog(INFO,"catched what we wanted");
    	}
    }

    if(plantree->lefttree!=(Plan *)NULL){
        check_modify_plan_node(plantree->lefttree,top_init_plans);
    }
    if(plantree->righttree!=(Plan *)NULL)
    {
        check_modify_plan_node(plantree->righttree,top_init_plans);
    }
}
/*
 * add by dongqing 19 August
 */


void
debug_target_list(List * targetlist){


    if(list_length(targetlist)==0) {
        elog(INFO,"null target");
    }
    else{
        ListCell * lf;
        int i=0;
        foreach(lf,targetlist){
            Node * lfn=(Node *) lfirst(lf);
            TargetEntry * te=(TargetEntry *) lfn;
            i++;
            if(!te->resjunk){
                //elog(INFO,"target_entries %d: %s",i,te->resname);
                //elog(INFO,"source oid:%d",te->resorigtbl);
            }
...
void
double_check(List * targetlist,char * targetone, char * targettwo, int * oid){
    int tempidone,tempidtwo;
    ListCell * lf;
    ...
    foreach(lf,targetlist)
    {
        ...
        //elog(INFO,"%s",te->resname);
        if(strcmp(te->resname,targetone)==0){
            tempidone=te->resorigtbl;
            //elog(LOG,"%d",tempidone);
        }

        if(strcmp(te->resname,targettwo)==0){
            tempidtwo=te->resorigtbl;
        }

        if(tempidone==tempidtwo && tempidone!=-1){
            break;
        }

    }
    if(tempidone!=-1 && tempidone==tempidtwo){
        *oid=tempidone;
    }
    else{
        elog(INFO,"can't find the target's rename oid");
    }
}

void
find_anno_table(List * targetlist){
    double_check(targetlist,"author","timestamp",&anno_table_oid);
    //elog(INFO,"anno_table_id:%d",anno_table_oid);

    double_check(targetlist,"tuple_id","tuple_column",&data_anno_oid);
    //elog(INFO,"data_anno_id:%d",data_anno_oid);


}
void
remove_targetlist(List * targetlist){
    ...
    foreach(lf,targetlist){
        ...
        int resorigtbl=te->resorigtbl;

//        if(resorigtbl==anno_table_oid || resorigtbl==data_anno_oid){
//            te->resjunk=true;
//        }
        /*
         * here,waiting for check! don't references something not exists
         *
         */
        if(te->resname==NULL){return;}
        if(strcmp(te->resname,"tuple_id")==0|| strcmp(te->resname,"td_sr")==0){
        	//te->resjunk=true;
        }
        //if(te->resorigtbl==anno_table_oid || te->resorigtbl)
...
void
top_remove_targetlist(List * targetlist){

	ListCell * lf;
	//elog(INFO,"top remove");
	 if(targetlist==NULL) return;
	    ...
	    foreach(lf,targetlist){
	        ...
	        if(te->resname==NULL){return;}
	        //printf("te->resname:%s \n",te->resname);
//	       // if(strcmp(te->resname,"tuple_id")==0|| strcmp(te->resname,"td_sr")==0||strcmp(te->resname,"annos")==0){
//	        	//elog(INFO,"top remove based on name");
//	        	te->resjunk=true;
//	        }
	       //clear

//	        if(strcmp(te->resname,"annos")==0){
//	        	te->resjunk=true;
//	        }
...
void
basic_remove(Plan * plantree, bool * found){

	if(!plantree) return;
    Plan * lefttree=(Plan *) plantree->lefttree;
    Plan * righttree=(Plan *) plantree->righttree;
    ...
    leftfound=false;
    rightfound=false;
    if(lefttree!=(Plan *) NULL){
        basic_remove(lefttree,&leftfound);
    }

    if(righttree!=(Plan *) NULL){
        basic_remove(righttree,&rightfound);

    }

    ptype=plantree->ptype;
    //elog(INFO,"ptype:%d",ptype);

    if(ptype>0){
        //elog(INFO,"one");
        *found=true;
        remove_targetlist(plantree->targetlist);
        if(debug_check_modify_plan_node){
        	debug_target_list(plantree->targetlist);
        }
        return;
    }
...
/*travel plan node tag and print it out for debuging*/
void travel_plan_node_tag(Node * plannode,int level){
	if(plannode==NULL){
			return;
		}
	Node * left_plan=(Node *) ((Plan *) plannode)->lefttree;
	Node * r_plan=(Node *) ((Plan *) plannode)->righttree;

	 switch(nodeTag(plannode)){
	 case T_NestLoop:
		 printf("%d:nestloop -ptype %d \n",level,((Plan *) plannode)->ptype);
		 break;
	 case T_HashJoin:
		 printf("%d:hash join -ptype %d \n",level,((Plan *) plannode)->ptype);
		 break;
	 case T_MergeJoin:
		 printf("%d:MergeJoin\n",level);
		 break;
	 ...
	 case T_Hash:
		 ...
		 break;
	 case T_CteScan:
		 printf("%d:CteScan -ptype %d \n",level,((Plan *) plannode)->ptype);
		 break;
	 case T_Group:
		 printf("%d:group \n",level);
		 break;
	 case T_Agg:
		 printf("%d: T_agg  -ptype %d\n",level,((Plan *) plannode)->ptype);
		 break;
	 default:
		 printf("%d:now we not care: \n",level);
		 break;
	 }

	 level=level+1;
	 travel_plan_node_tag(left_plan,level);
	 travel_plan_node_tag(r_plan,level);
}


void travel_plan_node(Node * plannode,List * top_init_plans)
{
    ...
    switch(nodeTag(plannode))
    {
        ...
        case T_MergeJoin:{
            ...
            join_node=(Join *) plannode;
            inner_plan=(Node *) join_node->plan.lefttree;
            outer_plan=(Node *) join_node->plan.righttree;



            leftcte_id=found_cte_scan(inner_plan);
            rightcte_id=found_cte_scan(outer_plan);

            flag=check_init_plans(top_init_plans,leftcte_id,rightcte_id);

            if(flag!=0){
                Plan * plan=(Plan *) plannode;
                plan->ptype=flag;
                //elog(INFO,"changed:%d",plan->ptype);
            }else{
            	Plan * plan=(Plan *) plannode;
            	plan->ptype=0;
            }
        ...
        case T_Agg:{
				elog(INFO,"T_Agg");
				elog(INFO,"waitin for do something \n");
        ...
        default:
        {
            ...
            Node * left_plan=(Node *) ((Plan *) plannode)->lefttree;
            Node * r_plan=(Node *) ((Plan *) plannode)->righttree;
            travel_plan_node(left_plan,top_init_plans);
            travel_plan_node(r_plan,top_init_plans);
        }
    }
}

int found_cte_scan(Node * plannode){

    int cte_id=-1;
    if(plannode==NULL) return cte_id; // nothing we need to do;

    switch(nodeTag(plannode)){
        case T_CteScan:
            //CteScan * ctescan_node=(CteScan*)plannode;
            //cte_id=ctescan_node->ctePlanId;
            cte_id = ((CteScan *) plannode)->ctePlanId;
            return cte_id;
            break;
        case T_Sort:
        case T_Hash:
        {
            ...
            n_plan_node=(Plan *) plannode;
            inner_plan=(Node *)n_plan_node->lefttree;
            outer_plan=(Node *)n_plan_node->righttree;
...
void
check_modify_remove_plan_node(Plan * plantree)
{
	bool found;
    find_anno_table(plantree->targetlist);
   // top_change(plantree);
    // set the anno_table_id
   // travel_plan_node_tag((Node*) plantree,1);
    travel_plan_node((Node *) plantree, plantree->initPlan);
    // found init_anno_tuples plan node, mark them
    found=false;
    basic_remove(plantree,&found);
    travel_plan_node_tag((Node*) plantree,1);
}

/*update by dongqing 14,9*/


void
changeDA(Plan * daplan){
	Node * plannode=(Node *)daplan;

	switch(nodeTag(plannode)){
			case T_Agg:
				daplan->ptype=-1;
				printf("T_agg: -ptype %d \n",daplan->ptype);
				break;
			case T_CteScan:
				daplan->ptype=-1;
				printf("T_cteScan: -ptype %d \n",daplan->ptype);
				break;
			default:
	...
	Plan * leftplan=daplan->lefttree;
	Plan * rightplan=daplan->righttree;
...
void
find_tempDACTE(List * top_init_plans,int * temp_daid){
	ListCell * lf;
	if(top_init_plans==(List *)NULL) return;

	foreach(lf,top_init_plans)
	{
			Node * plannode=(Node *) lfirst(lf);
			if(IsA(plannode,SubPlan))
			{

				SubPlan * isub=(SubPlan *)plannode;
				//printf("subPlan \n");
				if(find(isub->plan_name,"CTE temp_da")!=-1){
					*temp_daid=isub->plan_id;
					printf("subplan name:%s\n",isub->plan_name);
					return;
...
void
find_DACTE(List * top_init_plans, int * daid){
	ListCell * lf;
	if(top_init_plans==NULL) {return;}
	...
	foreach(lf,top_init_plans){
		...
		if(IsA(plannode,SubPlan)){
			...
			SubPlan * isub=(SubPlan *)plannode;
			printf("subPlan \n");
		if(find(isub->plan_name,"CTE da")!=-1){
			*daid=isub->plan_id;
			printf("subplan name:%s\n",isub->plan_name);
			return;
			//elog_node_display();
			/*
			printf("travel da plan \n");
			CteScan * scanplan=(CteScan *)isub;
			Scan sscan=scanplan->scan;

			Plan * inle=sscan.plan.lefttree;
			Plan * onle=sscan.plan.righttree;
			elog(INFO,"DAPlan",&sscan,true);

			if(inle!=NULL){
				elog(INFO,"left not null");
			}
			else{
				elog(INFO,"sub left NULL");
			}

			if(onle!=NULL){
				elog(INFO,"sub right not NULL");
			}
			else{
				elog(INFO,"sub right NULL");
			}
			*/

			//elog_node_display(INFO," from sub plan DA_plan_leftree",inle,true);
			//elog_node_display(INFO,"from sub plan DA_plan right tree",onle,true);
			//travel_plan_node_tag((Node *)isub,2);
			//elog_node_display(INFO,"DA_plan left tree",((Plan *)isub)->lefttree, true);
			//elog_node_display(INFO,"DA_plan right tree",((Plan *)isub)->righttree,true);
...
void
top_find_DActe(Plan * plantree,int* daid ){

	Node * topplannode=(Node *) plantree;
	switch(nodeTag(topplannode))
	{
		case T_NestLoop:{
			Node * leftplan=(Node *)plantree->lefttree;
			Node * rightplan=(Node *)plantree->righttree;
			find_DACTE(plantree->initPlan,daid); // here we need to know it is left or right tree

			if(IsA(leftplan,CteScan)){
				int leftid=((CteScan*)leftplan)->ctePlanId;
				if(leftid==(*daid)){
					plantree->ptype=12;
					printf("left tree is the one we need \n");
					((Plan *) leftplan)->ptype=-1;
				}
			}
			if(IsA(rightplan,CteScan)){
				int rightid=((CteScan*)rightplan)->ctePlanId;
				if(rightid==(*daid)){
					((Plan *)topplannode)->ptype+=1; // right one is ...
					plantree->ptype=11;
					((Plan *) rightplan)->ptype=-1;
					}
			}
			printf("current plan:ptype:%d \n",((Plan *)topplannode)->ptype);
...
void
top_change(Plan * plantree){
	/*change the top one's ptype and remove something!*/
	plantree->ptype=11; // special for the summmary_dealing
	top_remove_targetlist(plantree->targetlist);
}



void
sp_check_modify_remove_plan_node(Plan * plantree){
	top_change(plantree);
}


/*
void travel_sub_plans(List * subplans){

	ListCell * lf;
	int i=0;
	foreach(lf,subplans){
		Plan * splan=(Plan *) lfirst(lf);
		//elog(INFO,"subplans of")
		i+=1;
		if(i==5){
			elog_node_display(INFO,"subplans fo planned stmt",splan,true);
			//travel_plan_node_tag((Node *)splan,2);
			Node * righttree=(Node *) splan->righttree;
			if(IsA(righttree,Hash)){
				elog(INFO,"hash");
				Hash * hplan=(Hash *)righttree;
				Node * lplan=(Node *)hplan->plan.lefttree;
				if(IsA(lplan,CteScan)){
					int cteid=((CteScan*)lplan)->ctePlanId;
					elog(INFO,"cteid:%d",cteid);
				}
			}
	...
	//elog(INFO,"has %d sub plans",list_length(subplans));
}
*/

Plan * return_DA_plan(List * subplans,int daid){
	ListCell * lf;

	int i=0;
	if(i>list_length(subplans)){
		printf("we can't find the DA plan based on DAID");
		return NULL;
	}
	if(daid<0){
		return NULL;
	...
	foreach(lf,subplans){
		i=i+1;
		if(i==daid){
			Plan * daplan=(Plan *) lfirst(lf);
			printf("we found the da plan \n");
			return daplan;
		}
	}
	return NULL;
 ...
 */
void
check_modify_plan_list(List * plantree_list){
	ListCell * plantree_item;
	if(plantree_list==(List *)NULL){
		elog(ERROR,"something wrong passed");
		return;
	}
	 foreach(plantree_item,plantree_list)
	 {
	       ...
	       if(IsA(plannode,PlannedStmt))
	       {
			   PlannedStmt * planstmt=(PlannedStmt *) plannode;
			   //elog_node_display(INFO,"plannedstmt:",planstmt,true);
			   check_modify_remove_plannedstmt(planstmt);
	       }else{
	    	   return;
	       }
...
void
check_modify_remove_plannedstmt(PlannedStmt * planstmt){
	Plan * plantree;
	Plan * daplan;
	Plan * tempdaplan;
	int daid;
	...
	//travel_plan_node_tag((Node *)plantree,1);
	// I need to check what happen to analyze

	if(mode==3){
			if(plantree==(Plan *)NULL){
						return;
			}
							//check_modify_remove_plan_node(plantree);
			sp_check_modify_remove_plan_node(plantree);
			top_find_DActe(plantree,&daid);
			find_tempDACTE(plantree->initPlan,&temp_daid);
			daplan=return_DA_plan(planstmt->subplans,daid);
			tempdaplan=return_DA_plan(planstmt->subplans,temp_daid);
			if(tempdaplan){
				//printf("change the daplan's ptype =-1 \n");
				changeDA(daplan);
				//printf("travel the plan_node_tag of temp_da \n");
				travel_plan_node((Node *)tempdaplan,plantree->initPlan);
				basic_remove(tempdaplan,&tfound);
			}
			else{

				//printf("travel the plan node of da \n");
				travel_plan_node((Node *)daplan,plantree->initPlan);
				//printf("basic move of da \n");
				basic_remove(daplan,&found);
			}
			//printf("after basic_remove  plan tree \n");
			//travel_plan_node_tag((Node *)plantree,1); // debugging
			//printf("--------------- print the da plan\n");
			//travel_plan_node_tag((Node *)daplan,2); // debugging
			//printf("------------ print the tempdaplan \n");
			//travel_plan_node_tag((Node *)tempdaplan,4);
	}
	...
	if(mode==2){
		...
		if(tempdaplan){
			//printf("------------ print the tempdaplan \n");
			travel_plan_node_tag((Node *)tempdaplan,3);
		}
	}
...
/*
 * add by dongqing 11 Sep
 *
 */
/**
 * this function only server for debugging
 */

void
plan_from_query(char * query, PlannedStmt * stmt){
	...
	lf=list_head(parsetree_list);
	parsetree=(Node *) lfirst(lf);
