...
/*
 * print_tl
 *	  print target list in a more legible way.
 */
void
...
print_talist(List * tlist){
	...
	i=0;
	tlength=list_length(tlist);
	attrarray=(int *)malloc(tlength*sizeof(int));
		...
		foreach(tl, tlist)
		{
			...
			printf("\t resorigtbl:%d resorigcol%d \t",tle->resorigtbl,tle->resorigcol);

			if(tle->resorigcol!=0){
				attrarray[i++]=tle->resorigcol;
			}
	...
	for(index=0;index<i;index++){
		printf("%d",attrarray[index]);
	}

}

List *
get_talist(List * tlist){
	ListCell   *tl;
	List * attrl=(List *) NULL;
	...
	foreach(tl, tlist)
		{
			...
			if(tle->resorigcol!=0){
				//attrarray[i++]=tle->resorigcol;
				attrl=lappend_int(attrl,tle->resorigcol);
			}
		}
...
void
print_projectioninfo(ProjectionInfo * projInfo){
		...
		printf("print projection information \n");
		if (numSimpleVars > 0)
		    {
