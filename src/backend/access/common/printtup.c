...
void
Ptypedebugtuple(TupleTableSlot * 	slot, TupleTableSlot * 	aslot){

	TupleDesc typeinfo=aslot->tts_tupleDescriptor;
	int natts=typeinfo->natts;
	Form_pg_attribute  * attrs=typeinfo->attrs;
	int i;
	int target;
	...
	printf("complex debug here \n");
	for(i=0;i<natts;++i){
		if(strcmp(attrs[i]->attname.data,"td_sr")==0){
				target=i;
				break;
		}
	}
	origattr = slot_getattr(aslot, target+1, &isnull);
	if (isnull){
			printf("minus one \n");
	}
		else{
			 getTypeOutputInfo(typeinfo->attrs[target]->atttypid,
			                          &typoutput, &typisvarlena);
			 if (typisvarlena)
			             attr = PointerGetDatum(PG_DETOAST_DATUM(origattr));
			         else
			             attr = origattr;
			value = OidOutputFunctionCall(typoutput, attr);
			init_anno_list(slot->tts_summary,value);
			init_reps_from_anno_list(slot->tts_summary);
		}

}

void
UnionAnnotuple(TupleTableSlot * slot,List * ex_anno){
	if(ex_anno==(List *)NULL) {
		//printf("unionAnnotuple while is null");
		return;
	}


	slot->tts_summary->anno_list=list_union(slot->tts_summary->anno_list,ex_anno);

}


char *
getAttrsStr(TupleTableSlot *slot){
			char generalValue[10240];
			memset(generalValue,0,1024);
			TupleDesc typeinfo=slot->tts_tupleDescriptor;
			int natts=typeinfo->natts;
			Form_pg_attribute  * attrs=typeinfo->attrs;
			int i;
			...
			for(i=0;i<natts;++i){
				...
				if(strcmp(attrStr,"td_sr")!=0 && strcmp(attrStr,"tuple_id")!=0){
					strcat(generalValue,attrStr);
					strcat(generalValue,":");
					strcat(generalValue,value);
					if(i!=natts-1){
						strcat(generalValue,"||");
					}
				}
...
char *
getannossummary(TupleTableSlot *slot){
		TupleDesc typeinfo=slot->tts_tupleDescriptor;
		int natts=typeinfo->natts;
		Form_pg_attribute  * attrs=typeinfo->attrs;
		int i;
		...
		for(i=0;i<natts;++i){
			...
			if(strcmp(attrs[i]->attname.data,"annos")==0){
				target=i;
				break;
			}
		}
		origattr = slot_getattr(slot, target+1, &isnull);
		if (isnull){
			printf("null slot");
		}
		else{
...
void
debugtuple(TupleTableSlot * 	slot){
	TupleDesc typeinfo=slot->tts_tupleDescriptor;
	int natts=typeinfo->natts;
	Form_pg_attribute  * attrs=typeinfo->attrs;
	int i;
...
void
print_datum(Datum value, bool typByVal, int typLen)
{
