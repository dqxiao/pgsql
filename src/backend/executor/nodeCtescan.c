...
CteScanNext(CteScanState *node)
{
	...
	int ptype=node->ss.ps.plan->ptype;

//	if(ptype!=0){
//		printf("call CteScanNest \n");
//	}
	/*
...
//		}
		...
		slot=ExecCopySlot(slot, cteslot);

		if(ptype!=0){
							//elog(INFO,"exec Copyslot");
							slot->tts_summary=(Summary_tuple)malloc(sizeof(Summary_tuple));
							slot->tts_summary->anno_list=(List *) NULL;
							slot->tts_summary->reps=(List *) NULL;
							if(cteslot->tts_summary){
								List * cteanno=cteslot->tts_summary->anno_list;
								//printf("here we still have the summary \n");
								//printf("ctescan next have %d annos",list_length(cteanno));
								//elog(INFO,"union for ctes");
								UnionAnnotuple(slot,cteanno);
							}
							else{
								printf("before we copy, we have nothing \n");
							}
						}
		/*
		if(slot->tts_summary && ptype!=0){
			//printf("after copy! everything is ok \n");
		}
		else{
			if(ptype!=0){
				printf("copy ruin our life \n");
			}
		}
		*/
		return slot;
