			...
			DestReceiver *dest)
{
	...
	/*
	 * initialize information we need;
	 */
	ptype=planstate->plan->ptype;
	sub=1;
	...
	if(planstate->righttree !=NULL && ptype>10){
		PlanState * tempplanstate=innerPlanState(planstate);
		printf("Call Multi_plan\n");
		sub=Multi_plan(tempplanstate);
		ExecReScan(tempplanstate);
	}
	/*here,if the result need to be cached, then first allocate one List space for this result then store it
	 * with summary_tuples please*/
...
//		if((current_tuple_count % sub)!=0 && ptype!=0)
//			continue;
//
//		if(slot->tts_summary && ptype>10){
//			fillsummary(slot->tts_summary);
//		}

//		if(!execption){
//			putTupleIntoCache(slot);
//		}
		//printCachedLength();

//		if(slot->tts_summary){
//			travel_summary(slot->tts_summary);
//		}
		/*
...
int Multi_plan(PlanState * outernode){
	int i=0;
	TupleTableSlot *slot;
	char * value;
	...
	for(;;){
		...
		// here build one hash table for further processing
		value=getannossummary(slot);
		insertHash(value);
