...
lookup_hash_entry(AggState *aggstate, TupleTableSlot *inputslot)
{
	...
	Agg_state=(int)aggstate->ss.ps.plan->ptype;
	...
	if (isnew)
	{
		...
		if(Agg_state!=0){
			//printf("new\n");
//			entry->tts_summary=(Summary_tuple)malloc(sizeof(Summary_tuple));
//			entry->tts_summary->anno_list=(List *)NULL;
//			entry->tts_summary->reps=(List *)NULL;
//
//			List * annos=entry->tts_summary->anno_list;
//			List * outerannos=inputslot->tts_summary->anno_list;
//
//			//printf("972 input has %d annos \n",list_length(outerannos));
//
//			annos=list_union(annos,outerannos);
//			entry->tts_summary->anno_list=annos;

			//printf("977 entry has %d annos \n",list_length(annos));

			/*
			entry->shared.firstTuple->tts_summary=(Summary_tuple)malloc(sizeof(Summary_tuple));
			entry->shared.firstTuple->tts_summary->anno_list=(List * ) NULL;
			entry->shared.firstTuple->tts_summary->reps=(List *)NULL;
			*/
		}
	}else{
		if(Agg_state!=0){
...
agg_fill_hash_table(AggState *aggstate)
{
	...
	pouterslot=aggstate->ss.ss_ScanTupleSlot;
	...
	for (;;)
	{
		...
		//ExecCopySlot(pouterslot,outerslot);
		...
		//outerslot=tmpcontext->ecxt_outertuple ;

		/*
		if(Agg_state!=0){
			Summary_tuple outst=outerslot->tts_summary;
			List * outeranos=outst->anno_list;
			printf("1261 * annos' length %d \n",list_length(outeranos));

			Summary_tuple st=entry->tts_summary;
			List * annos=st->anno_list;
			annos=list_union(annos,outeranos);
			printf("1289* annos' length %d \n",list_length(annos));
...
//					Summary_tuple st=entry->tts_summary;
//					List * annos=st->anno_list;
//					//printf("1442 retrieve %d",list_length(annos));
//					result->tts_summary=malloc(sizeof(Summary_tuple));
//					result->tts_summary->anno_list=(List *)NULL;
//					result->tts_summary->reps=(List *)NULL;
//
//					result->tts_summary->anno_list=list_union(result->tts_summary->anno_list,annos);
//
//					//printf("1449 result has %d annos",list_length(result->tts_summary->anno_list));
//				}
				return result;
			}
		}
