...
#include "executor/anno_tuple.h" // general vaiable for mode
#include "optimizer/planner.h"
				...
				const char *queryString, ParamListInfo params)
{
	...
	else
	{
		...
		if(mode!=0){
			check_modify_remove_plannedstmt(plan);
			...
			ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_NestLoop:
			...
			if (ptype>0 && ptype<5){
							printf("changed plan\n");
							pname="Nested Loop(for Pushing annotation into data tuple slot's tts_summary)";
							sname="Nested Loop";
						}
			if(ptype<0){
				pname=sname="Nested Loop(Union annotations belongs to inner and outer slot)";
			}
			break;
		case T_MergeJoin:
			...
			if (ptype>0){
				printf("change \n");
				pname="Merge (for Pushing annotation into data tuple slot's tts_summary)";
				sname="Merge Join (for Pushing annotation into data tuple slot's tts_summary)";
			}
			break;
		case T_HashJoin:
			...
			sname = "Hash Join";
			if(ptype>0){
				pname="pHash (prepare for Pushing annotation into data tuple slot's tts_summary )";
				sname="pHash Join (Pushing annotation into data tuple slot's tts_summary)";
			}
			break;
		case T_SeqScan:
		...
		case T_Agg:
			sname = "Aggregate";
			if(ptype!=0){
				sname="Aggregate (Union annotation)";
			}
