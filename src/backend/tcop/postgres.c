...
#include "optimizer/planner.h" //check and modify plan_list
#include "pgstat.h"
#include "pg_trace.h"
...
/*add by dongqing*/
#include "parser/sql_translator.h" // translatio into compund liar
#include "executor/anno_tuple.h" //get the propagation mode
...
/*debuging propagation usage!*/
bool debug_propagate=true;
...
static void exec_propagation_query(const char *);
...
exec_simple_query(const char *query_string)
{
	...
	bool		isTopLevel;
	char		msec_str[32];
	char * promode=propagation_mode;



	/*deal with mode*/

	if(strcmp(promode,"standard")==0) mode=0;
	if(strcmp(promode,"standard-propagation")==0) mode=1;
	if(strcmp(promode,"summary-aware")==0) mode=2; // strategy 2
	if(strcmp(promode,"hybrid")==0)mode=3; // strategy 3 hybrid
	...
	query_string=translator(query_string,mode); // Understanding
	...
	foreach(parsetree_item, parsetree_list)
	{
		 ...
		 *
		 * Switch to appropriate context for constructing query trees (again,
		...
		if(plantree_list==(List *)NULL){
			elog(ERROR,"something stupid happens");
		}
		/*
		 * check and modify plan list
		 */
		printf("exec simple cmd mode :%d \n",mode);
		printf("typeofQuery:%d \n",typeofQuery(debug_query_string));

		if(mode!=0 && typeofQuery(debug_query_string)==1 ){
			check_modify_plan_list(plantree_list);
		}
...
/*
 * exec_propagation_query
 */
static void
exec_propagation_query(const char * query_string){

	/*support zoom_in query
	*/
...
//		ZoomInCmd zcmd=(ZoomInCmd) palloc(sizeof(ZoomInCmdData));
//		initZoomInCmd(zcmd);
//		zoomCmdParser(query_string,zcmd);
//		zoomInexecutor(zcmd);
	}
}
			...
			case 'Q':			/* simple query */
				{
					...
					exec_propagation_query(query_string);
...
void
SelfExecuteQuery(const char *query_string){
	execption=TRUE;
	exec_simple_query(query_string);
	execption=FALSE;
}
